/****************************************************
* FILE NAME : bitarray.c
*
* PURPOSE : Functions bitarray.
*
* MAINTAINER: Or Kol
*
* Reviewer: Noam Anholt
*
...
#define 32_BIT_SYS (32)

unsigned char set_bit_lut[BIT_LUT_SIZE] = {0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4};
unsigned char miror_bit_lut[BIT_LUT_SIZE] = {0, 8, 4, 12, 2, 10, 6, 14, 1, 9, 5, 13, 3, 11, 7, 15};
...
int BitArrCountSet(bitarr_t arr)
{
	if (32_BIT_SYS == SYS_BITS)
	{
...
{
	return SYS_BITS - BitArrCountSet(arr);
}
...
int BitArrCountSetLUT(bitarr_t arr)
{
	int counter = 0;
	
	while (0 != arr)
	{
		counter += set_bit_lut[arr & 0xF];
		arr >>= NIBBLE_SIZE;
	}
...
bitarr_t BitArrMirrorLUT(bitarr_t arr)
{
	...
	for ( ; i < BIT_LUT_SIZE; ++i)
	{
		...
		nibble_mir = miror_bit_lut[nibble];								/* get mirrored nibble from LUT*/
